...
export function createApp(deps: ToolDeps, opts: { authToken?: string }) {
  ...
  const handleMcp = async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.error('Erro ao processar requisição MCP', { error: (err as Error).message });
      if (!res.headersSent) {
